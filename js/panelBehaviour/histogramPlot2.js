...
function Histogram2 () {
    ...
    histogram2.initialize = function () {
        
        if (!selectedDimension2.isVisible) {
    ...
    histogram2.updateColoration = function() {
        updateYScale();        
        updateYAxis();       
        updatehistogramData();
        updateXScale();
    ...
    histogram2.hideDisplay = function () {
        _warningMessage.attr("visibility", "visible");
        _gPlot.attr("visibility", "hidden");
    }

    histogram2.updateScalesAndDisplay = function() {       
    ...
    histogram2.updateSelectedDimensions = function() {
    ...
    function updateYScale() {
        ...
        var dimension = selectedDimension2;
    ...
    function updatehistogramData() {
        ...
        _bins = histogram2(displayedData.filter( function(d) {return !d[coloredDimension.key]}));
        ...
        if (coloredDimension.property == "referential") {
            ...
            categories.forEach(function (cat) {
                _binsList.push(histogram2(displayedData.filter( function(d) {return d[coloredDimension.key] === cat} )));
            });
        }
        else {
            _binsList.push(histogram2(displayedData.filter( function(d) {return d[coloredDimension.key]})));
        }
    ...
    function updateBins() {
        ...
        // _gPlot.selectAll("path").remove();
        
        // Update histogram2 rectangles
        ...
        //             .datum(b)
        //             .attr("fill", "none")
        //             .attr("stroke", coloredDimension.colorScale(coloredDimension.domain[i]))
        //             .attr("stroke-width", 1.5)
        //             .attr("d", d3.line()
        //             .curve(d3.curveBasis) // Just add that to have a curve instead of segments
        //                 .x(function(d) { return _xScale(d.length) })
        //                 .y(function(d) { return _yScale(d.x1) })
        //             )
        //     });
